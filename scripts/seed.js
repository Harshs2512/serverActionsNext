...
async function seedBlog(client) {
  try {
    ...
    const insertedBlogs = await Promise.all(
      blogs.map(
        (rev) => client.sql`
        INSERT INTO blogs (id, title, description, slug, content, image_url)
        VALUES (${rev.id}, ${rev.title}, ${rev.description}, ${rev.slug}, ${rev.content}, ${rev.image_url}) ON CONFLICT (id) DO NOTHING;
      `,
      ),
    );
